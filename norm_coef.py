...
def normalize_dct(quantized_block, quant_table):
    """
    Нормализация (обратное квантование) коэффициентов ДКТ

    quantized_block - блок 8x8 после квантования (int)
    quant_table     - матрица квантования (та же, что использовалась при квантовании)
    """
    # Просто умножаем обратно
    normalized = quantized_block * quant_table
    return normalized.astype(np.float32)


def apply_normalization(quantized_blocks, quant_table):
    """Применяет нормализацию ко всем блокам"""
    h_blocks, w_blocks, _, _ = quantized_blocks.shape
    normalized_blocks = np.zeros((h_blocks, w_blocks, 8, 8), dtype=np.float32)

    for i in range(h_blocks):
        for j in range(w_blocks):
            normalized_blocks[i, j] = normalize_dct(quantized_blocks[i, j], quant_table)

    print(f"Нормализация (dequantization) выполнена для {h_blocks}x{w_blocks} блоков")
...
# Подготовка (используем то, что уже должно быть)
...
dct_blocks, C = apply_dct_matrix_to_image(Y)  # DCT через матрицы
quantized_blocks = apply_quantization(dct_blocks, quant_table_y)

print("\n=== До нормализации (квантованный блок) ===")
print(quantized_blocks[0, 0])

# Нормализация
normalized_blocks = apply_normalization(quantized_blocks, quant_table_y)

print("\n=== После нормализации ===")
print(np.round(normalized_blocks[0, 0], decimals=2))

# Сравнение с оригиналом после DCT
print("\nСредняя ошибка после квантования + нормализации:")
error = np.mean(np.abs(dct_blocks - normalized_blocks))
